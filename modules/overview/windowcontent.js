...
export const SearchAndWindows = (windows = true, search = true) => {
    ...
    const entry = Widget.Entry({
        ...
        onChange: (entry) => { // this is when you type
            ...
            if (resultsBox.get_children().length == 0) resultsBox.add(NoResultButton());
            resultsBox.show_all();
        },
    });
    const ov = windows ? overviewContent : Widget.Box({});
    const searchBox = search ? Widget.Box({
        hpack: 'center',
        children: [
            entry,
            Widget.Box({
    ...
    const results = search ? resultsRevealer : Widget.Box({});
    return Widget.Box({
        vertical: true,
        children: [
            searchBox,
        ...
        setup: (self) => self
            .hook(App, (_b, name, visible) => {
                if (name == 'overview' && !visible) {
                    resultsBox.get_children().forEach((child) => {
